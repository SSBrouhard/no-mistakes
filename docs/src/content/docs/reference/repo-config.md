...
Codex, Claude, Pi, and Grok (when `--no-context-files` is available) are the currently verified agents: Codex receives `project_doc_max_bytes=0` and `--ignore-rules`, Claude loads only its user setting source, Pi runs with `--no-context-files` (preserving a pinned `--no-context-files` or `-nc` spelling), and Grok runs with `--no-context-files` only when the installed CLI advertises that flag.
Official Grok without `--no-context-files` still discovers native project instructions and `.grok` project surfaces, so it is not a verified agent for this boundary. A configuration that resolves that Grok while this option is enabled therefore fails closed before launch rather than passing an unknown flag.
